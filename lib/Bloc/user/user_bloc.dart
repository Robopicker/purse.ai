...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  getRes(event) {
     if (event is UserLogin) {
      print(event.data);
      return UserState(isLoggedIn: true, name: state.name, email: state.email, password: state.password);
    } else if (event is CreateAccount) {
      print(event);
      return UserState(isLoggedIn: false, name: event.name ,email: event.email, password: event.password);
    }
